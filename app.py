...
def simpan_ke_gsheets():
    """Menyimpan data session_state ke 3 sheet berbeda di GSheets (Database)"""
    try:
...
def proses_excel_cvi():
    """Membaca 3 sheet dari GSheets dan menyuntikkannya ke 3 sheet file Excel Aiken"""
    try:
        client = get_gsheet_client()
        ss = client.open_by_url(GSHEET_URL)
        wb = openpyxl.load_workbook("CVI Aiken Zuyy.xlsx")
        
        sheets_to_process = ["KEJELASAN", "RELEVANSI", "KESESUAIAN"]
        ...
        for sheet_name in sheets_to_process:
            ...
            for idx, row_data in enumerate(all_vals[3:]):
                ...
                if len(row_data) < 2 or not row_data[1]: continue
                
                ws_xl.cell(row=target_row, column=2, value=row_data[1])
                for col_idx, val in enumerate(row_data[2:38]):
                    try:
                        clean_val = int(float(val)) if val else 0
                        ws_xl.cell(row=target_row, column=3 + col_idx, value=clean_val)
                    except:
    ...
    for target in targets:
        for f_type, f_buf in target["files"]:
            if f_buf is not None:
                fname = f"Form Validasi Forgiveness_{nama_panelis}.docx" if f_type == "docx" else f"CVI_Aiken_Kumulatif_{nama_panelis}.xlsx"
                f_buf.seek(0)
...
if st.session_state.step == 0:
    st.markdown("""<div class='intro-card'><h1>Expert Judgement Portal</h1><p>Instrument Validation for Forgiveness Scale</p><div style='margin-top: 30px; font-weight: bold; border: 2px solid white; display: inline-block; padding: 10px 20px; border-radius: 50px;'>WELCOME</div></div>""", unsafe_allow_html=True)
    st.write("---")
    ...
    st.write("Nilai aitem dari aspek Kejelasan, Relevansi, dan Kesesuaian (Skor 1-4).")
    ...
    if st.button("Lanjut 🚀"):
        if not st.session_state.p_nama or not st.session_state.p_kerja: st.error("Lengkapi data!")
        else: move_step(2); st.rerun()

elif st.session_state.step in [2, 3, 4]:
    aspek_map = {2: "Pemaafan Diri", 3: "Pemaafan Orang Lain", 4: "Pemaafan Situasi"}
    ...
    for ind_name, items in data_aspek[aspek_aktif]:
        ...
        for txt in items:
            ...
            bg = "#059669" if is_done else "#334155"
            st.markdown(f"<div style='background-color: {bg}; padding: 15px; border-radius: 8px 8px 0 0; margin-top: 20px;'><b style='color: white;'>{'✅' if is_done else '⏳'} {txt}</b></div>", unsafe_allow_html=True)
            st.markdown(f"<div style='background-color: {'#10B981' if is_done else '#475569'}; padding: 10px 20px; border-radius: 0 0 8px 8px;'>", unsafe_allow_html=True)
    ...
    if st.session_state.step == 4: st.session_state.saran_global = st.text_area("Saran Keseluruhan:", value=st.session_state.saran_global)
    n1, n2 = st.columns(2)
    with n1: 
        if st.button("⬅️ Kembali"): move_step(st.session_state.step - 1); st.rerun()
    with n2:
        if st.button("Lanjut ➡️"): move_step(5 if st.session_state.step == 4 else st.session_state.step + 1); st.rerun()

elif st.session_state.step == 5:
    st.title("Konfirmasi")
    st.session_state.confirmed = st.checkbox("Data sudah benar.")
    if st.button("🚀 KIRIM", disabled=not st.session_state.confirmed):
        with st.spinner("Mengirim data..."):
            if simpan_ke_gsheets():
                ...
                w_buf = io.BytesIO(); doc.save(w_buf); w_buf.seek(0)
                kirim_telegram_multi(w_buf, ex_buf, st.session_state.p_nama)
                move_step(6); st.rerun()
            else: st.error("Gagal simpan!")

elif st.session_state.step == 6:
    st.balloons()
    st.markdown("<div class='thanks-card'><h1>Sukses! ✨</h1><p>Data terkirim.</p></div>", unsafe_allow_html=True)
