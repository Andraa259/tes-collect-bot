...
def sync_data(txt, key_type):
    # Callback untuk memastikan data tersimpan sebelum UI dirender ulang
...
st.markdown("""
    ...
    .white-card { background-color: #FFFFFF !important; color: #1E293B !important; padding: 25px; border-radius: 0 0 10px 10px; border: 1px solid #E2E8F0; margin-bottom: 30px; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1); }
    .white-card p, .white-card b { color: #1E293B !important; }
...
if st.session_state.step == 0:
    st.markdown("<div class='intro-card'><h1>Expert Judgement Portal</h1><p>Forgiveness Scale Development</p></div>", unsafe_allow_html=True)
    ...
    for ind_name, items in data_aspek[aspek_aktif]:
        ...
        for txt in items:
            ...
            h_style = "background-color: #DCFCE7; border-left: 6px solid #22C55E; border: 1px solid #BBF7D0; border-bottom: none;" if is_done else "background-color: #FFFFFF; border-left: 6px solid #E2E8F0; border: 1px solid #E2E8F0; border-bottom: none;"
            b_style = "background-color: #F0FDF4; border-left: 6px solid #22C55E; border: 1px solid #BBF7D0; border-top: none;" if is_done else "background-color: #FFFFFF; border-left: 6px solid #E2E8F0; border: 1px solid #E2E8F0; border-top: none;"
    ...
    with n2:
        if st.button("Lanjut ➡️" if st.session_state.step < 4 else "🚀 KIRIM"):
    ...
    if st.button("🚀 YA, KIRIM", disabled=not st.session_state.confirmed):
    ...
    st.balloons(); st.success("Terima Kasih! Data telah tercatat.")
