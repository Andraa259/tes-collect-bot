...
# STEP 2, 3, 4: SCORING
    ...
    for ind_name, items in data_aspek[aspek_aktif]:
        ...
        for txt in items:
            ...
            card_class = "white-card card-done" if is_done else "white-card"
            
            with st.container():
                st.markdown(f"<div class='{card_class}'>", unsafe_allow_html=True)
                st.write(f"**{txt}** {'✅' if is_done else ''}")
...
# STEP 6: THANKS
    ...
    st.balloons()
    st.markdown("<div class='thanks-card'><h1>Terima Kasih! ✨</h1><p>Kontribusi Anda telah tercatat.</p></div>", unsafe_allow_html=True)
