            ...
            if target_row <= 33:
                ...
                for aspek in [
                    ...
                    for _, items in data_aspek[aspek]:
                        for txt in items:
                            val = st.session_state.master_data.get(txt, {key_data: 0})[
                                key_data
                            ]
    ...
    for target in targets:
        for f_type, f_buf in target["files"]:
            if f_buf is not None:
                fname = (
                    f"Form Validasi Expert Judgement Forgiveness_{nama_panelis}.docx"
                    if f_type == "docx"
                    else f"CVI_Aiken_Kumulatif_{nama_panelis}.xlsx"
                )
                f_buf.seek(0)
                requests.post(
                    url,
                    data={
                        "chat_id": target["id"],
                        "caption": f"✅ {f_type.upper()} Masuk: {nama_panelis}",
                    },
...
if st.session_state.step == 0:
    ...
    st.write("---")
    st.write(
        "Selamat datang di sistem validasi instrumen. Partisipasi Anda sangat krusial dalam menjamin kualitas alat ukur penelitian ini."
    )
...
# STEP 1: PETUNJUK & IDENTITAS
    ...
    st.markdown(
        f"<div class='def-box'><b>Definisi Operasional:</b><br>{DEF_OP}</div>",
        unsafe_allow_html=True,
    )
    st.subheader("📝 PETUNJUK PENGISIAN")
    st.info("Mohon dibaca sebelum memberikan penilaian")
    st.write(
        "Sehubungan dengan upaya pengembangan instrumen penelitian mengenai tingkat pemaafan (forgiveness) pada mahasiswa, kami meminta Bapak/Ibu untuk menilai item-item yang telah kami susun, dari aspek :"
    )
    st.markdown(
        """
    ...
    * **Kesesuaian**: Kesesuaian aitem yang disusun sudah sesuai dengan indikatornya.
    ...
    st.write(
        "Penilaian dilakukan dengan memberikan angka 1-4. Skor **0** berarti Anda belum memberikan penilaian."
    )
    st.markdown(
        """
    ...
    st.write("---")
    st.session_state.p_nama = st.text_input(
        "Nama Panelis", value=st.session_state.p_nama
    )
    st.session_state.p_kerja = st.text_input(
        "Pekerjaan", value=st.session_state.p_kerja
    )
...
# STEP 2-4: PENILAIAN
    ...
    for ind_name, items in data_aspek[aspek_aktif]:
        st.markdown(
            f"<div class='indicator-header'>{ind_name}</div>", unsafe_allow_html=True
        )
        for txt in items:
            if txt not in st.session_state.master_data:
                st.session_state.master_data[txt] = {
                    "kj": 0,
                    "rel": 0,
                    "kes": 0,
                    "ket": "",
            ...
            st.markdown(
                f"<div style='{h_style} padding: 15px; border-radius: 8px 8px 0 0; margin-top: 25px;'><b style='color: white !important; font-size: 1.1rem;'>{icon} {txt}</b></div>",
                unsafe_allow_html=True,
            )
            st.markdown(
                f"<div style='{b_style} padding: 10px 20px; border-radius: 0 0 8px 8px;'>",
                unsafe_allow_html=True,
            )
            ...
            st.session_state.master_data[txt]["ket"] = st.text_input(
                "Keterangan per Aitem:", value=d["ket"], key=f"ket_{txt}"
            )
    ...
    errors = [
        t
        for t in items_pg
        if any(st.session_state.master_data[t][k] == 0 for k in ["kj", "rel", "kes"])
    ]

    if st.session_state.step == 4:
        st.session_state.saran_global = st.text_area(
            "Catatan/Saran Keseluruhan:", value=st.session_state.saran_global
        )

    nav1, nav2 = st.columns(2)
    with nav1:
        if st.button("⬅️ Kembali"):
            move_step(st.session_state.step - 1)
            st.rerun()
    with nav2:
        btn_label = (
            "Lanjut ➡️" if st.session_state.step < 4 else "🚀 LANJUT KE PENGIRIMAN"
        )
        if st.button(btn_label):
            if errors:
                st.error(
                    f"⚠️ Ada {len(errors)} soal yang belum lengkap pada halaman ini."
                )
            else:
                move_step(
                    5 if st.session_state.step == 4 else st.session_state.step + 1
                )
                st.rerun()
...
# STEP 5: KONFIRMASI
    ...
    st.warning(
        "Mohon periksa kembali penilaian Anda. Setelah dikirim, data tidak dapat diubah."
    )
    ...
    nav1, nav2 = st.columns(2)
    with nav1:
        if st.button("⬅️ Kembali ke Penilaian"):
            move_step(4)
            st.rerun()
    with nav2:
            ...
            with st.spinner("Sedang memproses..."):
                try:
                    ...
                    for p in doc.paragraphs:
                        if "Nama\t\t:" in p.text:
                            p.text = f"Nama\t\t: {st.session_state.p_nama}"
                        if "Pekerjaan\t:" in p.text:
                            p.text = f"Pekerjaan\t: {st.session_state.p_kerja}"
                    table = doc.tables[0]
