...
if st.session_state.step == 0:
    ...
    st.write("Berikan penilaian dengan skor 1-4. Skor **0** berarti Anda belum mengisi.")
    ...
    if not st.session_state.submitted:
        with st.spinner("Mencatat data & Mengirim berkas..."):
            try:
                ...
                for aspect in ["Pemaafan Diri", "Pemaafan Orang Lain", "Pemaafan Situasi"]:
                    for _, items in data_aspek[aspect]:
                ...
                for ws_name, k in zip(worksheets, keys):
                    ...
                    # LOGIKA CARI BARIS KOSONG (Fix Baris Penuh)
                    idx_target = None
                    for i in range(2, 32): # Cek baris 3-32
                        if i >= len(df_old):
                            idx_target = i; break
                        
                        val_b = str(df_old.iloc[i, 1]).strip()
                        # Dianggap kosong jika "", "0", "nan", atau "None"
                        if val_b in ["", "0", "0.0", "nan", "None"]:
                            idx_target = i; break
                ...
                for row in table.rows:
                    ...
                    for txt_ori, data in st.session_state.master_data.items():
                        ...
                        if txt_normalized[:50] in aitem_word: # Ambil 50 karakter pertama saja untuk kecocokan
                            row.cells[3].text = str(data["kj"])
                            row.cells[4].text = str(data["rel"])
                            row.cells[5].text = str(data["kes"])
                            row.cells[6].text = str(data["ket"])

                # Tambahkan saran global di baris catatan
                for row in table.rows:
                    if "Catatan" in row.cells[2].text:
                        row.cells[2].text += f"\nSaran Panelis: {st.session_state.saran_global}"
    ...
    st.markdown("""
        <div class='thanks-card'>
            ...
            <p style='font-size: 1.2rem; color: #475569;'>Data penilaian Anda telah kami terima.</p>
            <hr>
            <p style='font-style: italic; color: #64748b;'>Halaman ini dapat ditutup.</p>
        </div>
    """, unsafe_allow_html=True)
